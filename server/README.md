...
| `GET` | `/status` | 抓取状态：`{worker: running|halted, halted?}`（拉黑停摆时供 MCP/前端感知） |
| `POST` | `/restart` | 清暂停标志、恢复抓取（拉黑解除/换出口 IP 后调用；进程不重启） |
| `GET` | `/healthz` | 健康检查（不触 baostock） |

**提交示例**（`priority`：`high` MCP 交互读插队 / `low` 后台全量同步，缺省 `low`）
...
- **单 worker 串行 + 高低优先** — 满足 baostock 单连接约束；worker `BRPOP [high, low]`，
  先清空 `high`（MCP 交互读插队）再消费 `low`（后台全量同步），逐个执行。
...
| db2 | job 存储 | `job:{id}`(hash 状态) / `job:result:{id}`(payload) / `job:idx:{params_hash}`(去重索引) / `fetch:pending`(low 队列) + `fetch:pending:high`(high 队列，优先消费) |
