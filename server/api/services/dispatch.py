...
def _wait_existing(params_hash: str, timeout: float, task_name: str) -> None:
    ...
    僵尸防护：running 行的 started_at 超过 task_time_limit×2 视为其子进程已被
    SIGKILL 且无人收尸，标记 failed 释放唯一索引，让客户端重试时能重新投递。
    """
    from datetime import datetime, timedelta, timezone

    deadline = time.monotonic() + timeout
    stuck_after = timedelta(seconds=settings.task_time_limit * 2)
    while time.monotonic() < deadline:
        with SyncSession() as s:
            row = s.execute(
                select(FetchTask.id, FetchTask.status, FetchTask.started_at)
        ...
        if (
            ...
            and datetime.now(timezone.utc) - row.started_at > stuck_after
        ):
