...
public static class IndexTools
{
    ...
    private static async Task<string> Constituents(StockDataApiClient api, string index,
        string? date, int limit, CancellationToken ct)
        => JsonHelper.Truncate(await api.GetAsync(
            $"/api/v1/indices/{index}/constituents",
    ...
    public static Task<string> GetIndexConstituents(StockDataApiClient api,
        ...
        => IndexAlias.TryGetValue(index.Trim(), out var key)
            ? Constituents(api, key, date, limit, ct)
            : Task.FromResult($"Error: 不支持的指数 '{index}'，可选: sz50/hs300/zz500");

    [McpServerTool(Name = "get_sz50_stocks")]
    [Description("获取上证50成分股。")]
    public static Task<string> GetSz50(StockDataApiClient api, string? date = null,
        int limit = 250, CancellationToken ct = default)
        => Constituents(api, "sz50", date, limit, ct);

    [McpServerTool(Name = "get_hs300_stocks")]
    [Description("获取沪深300成分股。")]
    public static Task<string> GetHs300(StockDataApiClient api, string? date = null,
        int limit = 350, CancellationToken ct = default)
        => Constituents(api, "hs300", date, limit, ct);

    [McpServerTool(Name = "get_zz500_stocks")]
    [Description("获取中证500成分股。")]
    public static Task<string> GetZz500(StockDataApiClient api, string? date = null,
        int limit = 550, CancellationToken ct = default)
        => Constituents(api, "zz500", date, limit, ct);

    [McpServerTool(Name = "get_stock_industry")]
    [Description("获取行业分类信息。code 为空时返回全部股票的行业分类。")]
    public static async Task<string> GetStockIndustry(StockDataApiClient api,
        string? code = null, string? date = null, int limit = 250,
        CancellationToken ct = default)
        => JsonHelper.Truncate(await api.GetAsync("/api/v1/industries",
            new() { ["code"] = code, ["snap_date"] = date }, ct), limit);

    [McpServerTool(Name = "list_industries")]
    [Description("列出全部行业名称及成分股数量。")]
    public static async Task<string> ListIndustries(StockDataApiClient api,
        string? date = null, CancellationToken ct = default)
    {
        var json = await api.GetAsync("/api/v1/industries",
    ...
    public static async Task<string> GetIndustryMembers(StockDataApiClient api,
