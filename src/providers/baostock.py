...
def _query_fina_category(bs_func, prefix: str, code: str, year: str, quarter: int) -> dict:
    ...
    try:
        ...
        if rs.error_code != '0' or not rs.next():
            return {}
        row = rs.get_row_data()
        return {f"{prefix}_{field}": row[i] for i, field in enumerate(rs.fields) if i < len(row)}
    except Exception as e:
...
class BaostockDataSource(FinancialDataSource):
    ...
    def get_dividend_data(self, code: str, year: str, year_type: str = "report") -> pd.DataFrame:
        """获取分红送转数据。"""
        return _query(
            bs.query_dividend_data, f"分红 {code} {year}",
    ...
    def get_adjust_factor_data(self, code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """获取复权因子数据。"""
        return _query(
            bs.query_adjust_factor, f"复权因子 {code} {start_date}~{end_date}",
    ...
    def get_profit_data(self, code: str, year: str, quarter: int) -> pd.DataFrame:
        """获取季度盈利能力数据。"""
        return _query(bs.query_profit_data, f"盈利能力 {code} {year}Q{quarter}", code=code, year=year, quarter=quarter)

    def get_operation_data(self, code: str, year: str, quarter: int) -> pd.DataFrame:
        """获取季度营运能力数据。"""
        return _query(bs.query_operation_data, f"营运能力 {code} {year}Q{quarter}", code=code, year=year, quarter=quarter)

    def get_growth_data(self, code: str, year: str, quarter: int) -> pd.DataFrame:
        """获取季度成长能力数据。"""
        return _query(bs.query_growth_data, f"成长能力 {code} {year}Q{quarter}", code=code, year=year, quarter=quarter)

    def get_balance_data(self, code: str, year: str, quarter: int) -> pd.DataFrame:
        """获取季度偿债能力数据。"""
        return _query(bs.query_balance_data, f"偿债能力 {code} {year}Q{quarter}", code=code, year=year, quarter=quarter)

    def get_cash_flow_data(self, code: str, year: str, quarter: int) -> pd.DataFrame:
        """获取季度现金流量数据。"""
        return _query(bs.query_cash_flow_data, f"现金流量 {code} {year}Q{quarter}", code=code, year=year, quarter=quarter)

    def get_dupont_data(self, code: str, year: str, quarter: int) -> pd.DataFrame:
        """获取季度杜邦分析数据。"""
        return _query(bs.query_dupont_data, f"杜邦分析 {code} {year}Q{quarter}", code=code, year=year, quarter=quarter)

    def get_performance_express_report(self, code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """获取业绩快报。"""
        return _query(
            bs.query_performance_express_report, f"业绩快报 {code} {start_date}~{end_date}",
            code=code, start_date=start_date, end_date=end_date,
        )
    ...
    def get_forecast_report(self, code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """获取业绩预告。"""
        return _query(
            bs.query_forecast_report, f"业绩预告 {code} {start_date}~{end_date}",
            code=code, start_date=start_date, end_date=end_date,
        )
    ...
    def get_trade_dates(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
        """获取交易日历。"""
        return _query(
            bs.query_trade_dates, f"交易日历 {start_date or '默认'}~{end_date or '默认'}",
            start_date=start_date, end_date=end_date,
        )
    ...
    def get_deposit_rate_data(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
        """获取基准存款利率。"""
        return _query(bs.query_deposit_rate_data, "基准存款利率", start_date=start_date, end_date=end_date)

    def get_loan_rate_data(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
        """获取基准贷款利率。"""
        return _query(bs.query_loan_rate_data, "基准贷款利率", start_date=start_date, end_date=end_date)
    ...
    def get_required_reserve_ratio_data(self, start_date: Optional[str] = None, end_date: Optional[str] = None, year_type: str = '0') -> pd.DataFrame:
        """获取存款准备金率。"""
        return _query(
            bs.query_required_reserve_ratio_data, "存款准备金率",
    ...
    def get_money_supply_data_month(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
        """获取月度货币供应量。"""
        return _query(bs.query_money_supply_data_month, "月度货币供应量", start_date=start_date, end_date=end_date)

    def get_money_supply_data_year(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
        """获取年度货币供应量。"""
        return _query(bs.query_money_supply_data_year, "年度货币供应量", start_date=start_date, end_date=end_date)

    # ── 指数与行业 ──

    def get_stock_industry(self, code: Optional[str] = None, date: Optional[str] = None) -> pd.DataFrame:
        """获取行业分类信息。"""
        return _query(bs.query_stock_industry, f"行业分类 {code or '全部'}", code=code, date=date)
    ...
    def get_hs300_stocks(self, date: Optional[str] = None) -> pd.DataFrame:
        """获取沪深300成分股。"""
        return _query(bs.query_hs300_stocks, f"沪深300成分股 {date or '最新'}", date=date)

    def get_zz500_stocks(self, date: Optional[str] = None) -> pd.DataFrame:
        """获取中证500成分股。"""
        return _query(bs.query_zz500_stocks, f"中证500成分股 {date or '最新'}", date=date)
